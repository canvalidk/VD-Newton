...
**Handoff to VD-docs — staged 2026-09-23; not yet received.**

- **Kind.** Record, requested by the user on 2026-09-23 ("start with staging
  contribution 12 as a record").
- **Intended home.** `Newton-analysis/_dscn_mass_estimator/`, as a flat member.
  Nothing in it asserts `supersedes` or `corrects`; it scopes earlier members
  (see its §§7 and 9).
- **Source.** VD-Newton
  `03_trace_and_evaluator/mass_estimator/investigation/prior_factors_and_sequential_updating.md`
  (working contribution 12), SHA-256 `f42dc362ae59d64e55c5cfe8dc2f39844b0b2abb02ee306ec078626d64e0e187`. The working tree is based on
  `canvalidk/VD-Newton@0525c5a`. The contribution, its code, tests and study
  protocols are not yet committed, so there is no source commit.
- **Links.** The relative links below point into that VD-Newton investigation
  folder and its `.tools/` scratch area; they do not resolve inside VD-docs.
  The `.tools/` outputs are regenerable from the protocols and code and are
  not part of this handoff.
- **Maths.** Not yet normalized for VD-docs intake.
