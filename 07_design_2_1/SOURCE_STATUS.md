...
| `08_nm_lesson_drafts/nml3/NML3_ENTRY_SEQUENCE_DECISION_2026-08-02.md` | Current NML3 numbering authority | E9 doubles as the Newton-II net-force triplet entry and force-sum chimney; there is no standalone force-sum chimney; E13-E17 are the force-sum triplet and walls; meta-typing moves to E36-E41 at the end of the active ledger. |
| `08_nm_lesson_drafts/nml3/NML3_CONTRIBUTING_FORCE_TERMINOLOGY_DECISION_2026-08-02.md` | Provisional force-member naming record | `contributing-force` is the consistent active working term, but final naming is reopened by the foundational NML3 note because algebraic summand status must not be confused with force-account membership. Read `attached-force` and `acting-force` in preserved historical sources as predecessor terms unless a source explicitly distinguishes them. |
| `08_nm_lesson_drafts/nml3/NML3_FORCE_SUM_FOUNDATIONAL_UNDERSTANDING_2026-08-02.md` | Current NML3 conceptual starting note | Records the force-sum lesson goal, the five construction hurdles, and the critical rule that an algebraic decomposition of net force does not establish force-account membership. The force-member headword remains open for reconsideration. |
...
| `03_trace_and_evaluator/vd_trace_level2_v3.md` | Trace style reference | Use for lazy demand and blocked-thunk style. |

## Historical Or Deferred Sources

| Source | Status | Use |
|---|---|---|
| `06_newton_analysis/nothing_vs_zero_somethings.md` | Superseded on routing | Keep only the empty-list / zero-sum intuition. Do not revive `Nothing -> Newton I`. |
