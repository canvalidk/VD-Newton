...
> **Naming decision — 26 August 2026:** the final entry-facing triplet names
> are `net-force / impressed-force / interaction-set`, governed by
> `../08_nm_lesson_drafts/nml3/NML3_FORCE_SUM_TRIPLET_NAMING_DECISION_2026-08-26.md`.
> In the historical derivation below, read `IFS` as `interaction-set`, read the
> indexed member as an interaction `i`, and read `V(i)` as the target-directed
> `impressed-force` supplied by that interaction. The derivation remains useful;
> its provisional names are no longer open choices.
