...
## Relationship to VD Folders

The private repository `canvalidk/VD-docs` is the VD project's cross-branch
document-management layer. It manages received documents through its inbox,
catalog, branch placement, draft history, provenance, backups, and informing or
AI-context copies. It is not another content/publishing branch alongside
Theory, Core, Code, Newton, and Newton-analysis, and it does not take over those
branches' substantive decisions.

VD-docs is authoritative for the organizational state of material it has
ingested, but it is not necessarily exhaustive of active work. This local
Newton folder may contain newer or provisional notes that have not yet been
submitted to the VD Folders inbox. For broad context questions, consult both
the managed VD-docs collection and relevant active local material; do not treat
absence from either location as proof that the other has nothing relevant.

This workspace owns entry production: entry packets, actual Newton entries,
and the trace/evaluator work needed to test them. Managed NML and analysis
documents are normally consumed as context rather than reorganized here.
...
- `README.md`
- `entry_writing_guide_auditable_compositions.md`
- `VD_NEW_ENTRY_TEMPLATE.md`
...
An **entry packet** means the copy/paste working card in
`04_entry_design_guides/VD_NEW_ENTRY_TEMPLATE.md`. Use it to gather the
information needed to draft or revise one VD entry; it is not itself the final
entry wording.
